...
export default function CheckoutPage() {
  ...
  const initialization = {
    amount: 1, // O valor real é controlado pelo preferenceId no backend
    preferenceId: preferenceId || '',
  };

  const onSubmit = async ({ selectedPaymentMethod, formData }: any) => {
    // Bricks gerencia a submissão via preferenceId automaticamente
          ...
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <header className="space-y-6 text-center">
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.6em] text-accent">Checkout Seguro</span>
                <div className="h-px w-12 bg-accent" />
              </div>
              ...
              <div className="lg:col-span-4 space-y-8">
                <div className="p-8 bg-secondary/20 rounded-[2rem] space-y-6 border border-primary/5">
                   <div className="flex items-center gap-4 text-primary">
                      ...
                      <h4 className="text-[11px] font-bold uppercase tracking-widest">Ambiente Blindado</h4>
                   </div>
                   <p className="text-xs text-muted-foreground leading-relaxed italic font-light">
                     Seus dados financeiros são processados com criptografia de ponta a ponta pelo Mercado Pago. A Toda Bela não armazena informações de cartões.
                   </p>
                </div>

                <div className="p-8 bg-primary text-white rounded-[2rem] space-y-6 shadow-2xl">
                   <div className="flex items-center gap-4">
                      <ShieldCheck className="h-6 w-6 text-accent" />
                      <h4 className="text-[11px] font-bold uppercase tracking-widest text-accent">Garantia Toda Bela</h4>
                   </div>
                   <ul className="space-y-4">
                     {[
                       "Processamento Imediato",
                       "Rastreio via WhatsApp",
                       "Seguro contra extravios",
                       "Suporte VIP Pós-venda"
                     ].map((item, i) => (
