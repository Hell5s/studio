    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-primary selection:text-white overflow-x-hidden">
      ...
      <main className="pt-[100px] md:pt-[120px]">
        ...
        <section className="py-24 md:py-40 container mx-auto px-6 text-center">
          <div className="max-w-4xl mx-auto space-y-10">
            ...
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-serif font-bold text-primary leading-tight tracking-tight">
              A Toda Bela nasce para vestir mulheres que valorizam presença, estilo e autenticidade em cada detalhe.
            </h2>
            <div className="h-0.5 w-24 bg-accent mx-auto opacity-30" />
          </div>
        </section>
        ...
        <section id="categorias" className="py-24 md:py-32 bg-white">
          <div className="container mx-auto px-6">
            <div className="text-center mb-20 space-y-4">
              ...
              <h2 className="text-4xl md:text-6xl font-serif font-bold text-primary">Encontre seu estilo</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <div key={cat.name} className="group relative aspect-[3/4] rounded-[3rem] overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-700">
                  <Image 
                    src={cat.img} 
                    alt={cat.name} 
                    fill 
                  ...
                  <div className="absolute bottom-10 left-10 text-white">
                    ...
                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-all duration-700 mt-2">Explorar Peças</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section id="vitrine" className="py-24 md:py-40">
          <div className="container mx-auto px-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-8">
              <div className="space-y-4">
                <span className="text-[10px] font-bold uppercase tracking-[0.6em] text-accent">Destaques do Momento</span>
                <h2 className="text-4xl md:text-7xl font-serif font-bold text-primary">Mais vendidos</h2>
              </div>
              <div className="flex flex-wrap gap-3">
                {["Novidades", "Vestidos", "Conjuntos", "Festa", "Casual"].map((category) => (
                  <button
                    key={category}
            ...
            {productsLoading ? (
              <div className="flex justify-center py-32"><Loader2 className="h-10 w-10 animate-spin text-accent" /></div>
            ) : (
        ...
        <section className="py-12 md:py-24">
          <div className="container mx-auto px-6">
            <div className="relative aspect-[21/9] rounded-[4rem] overflow-hidden group">
              <Image 
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8 space-y-6">
                <span className="text-[11px] font-bold uppercase tracking-[0.8em] text-white/80">Seleção Especial</span>
                <h3 className="text-4xl md:text-7xl font-serif font-bold text-white text-editorial">Destaques da semana</h3>
                <Button className="rounded-full bg-white text-primary px-12 h-16 text-[10px] font-bold uppercase tracking-widest hover:scale-105 transition-all shadow-2xl">
                  Ver produtos
                </Button>
              </div>
            </div>
        ...
        <section className="py-24 md:py-40 bg-white">
          <div className="container mx-auto px-6">
            <div className="text-center mb-20 space-y-4">
              <span className="text-[10px] font-bold uppercase tracking-[0.6em] text-accent">Vozes Toda Bela</span>
              <h2 className="text-4xl md:text-6xl font-serif font-bold text-primary">O que dizem nossas clientes</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-10">
              {[
                ...
                <div key={i} className="bg-background p-10 md:p-14 rounded-[3.5rem] border border-accent/10 space-y-8 hover:shadow-2xl transition-all duration-700 group">
                  <div className="flex gap-1 text-accent">
                    {[...Array(item.stars)].map((_, s) => <Star key={s} className="h-4 w-4 fill-current" />)}
                  </div>
                  <p className="text-base md:text-lg text-primary/70 italic leading-relaxed font-light">"{item.review}"</p>
                  <div className="flex items-center gap-5 pt-4">
                    <div className="h-12 w-12 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm shadow-lg group-hover:scale-110 transition-all">
                      {item.name[0]}
                    </div>
                    <span className="text-[11px] font-bold uppercase tracking-widest text-primary">{item.name}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Benefícios */}
        <section className="py-20 md:py-32 border-y border-accent/10 bg-white">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 text-center">
              {[
                { icon: <Truck className="h-6 w-6" />, title: 'Entrega Nacional', desc: 'Envio para todo o Brasil' },
                { icon: <ShieldCheck className="h-6 w-6" />, title: 'Compra Segura', desc: 'Dados 100% protegidos' },
                { icon: <Sparkles className="h-6 w-6" />, title: 'Peças Selecionadas', desc: 'Curadoria de alto padrão' },
                { icon: <HeadphonesIcon className="h-6 w-6" />, title: 'Atendimento Rápido', desc: 'Suporte especializado VIP' },
              ].map((benefit, i) => (
                <div key={i} className="space-y-4 group">
                  <div className="h-16 w-16 mx-auto rounded-full bg-secondary/40 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all duration-700 shadow-sm">
                    {benefit.icon}
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-primary">{benefit.title}</p>
                    <p className="text-[10px] text-primary/40 italic font-light">{benefit.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="bg-white pt-24 pb-12">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 text-left mb-20 max-w-6xl mx-auto border-t border-accent/10 pt-20">
            <div className="space-y-8">
              <h5 className="text-[11px] font-bold uppercase tracking-[0.4em] text-accent">Navegação</h5>
              <ul className="space-y-4 text-[11px] text-primary/60 font-medium">
            ...
            <div className="space-y-8">
              <h5 className="text-[11px] font-bold uppercase tracking-[0.4em] text-accent">Atendimento</h5>
              <ul className="space-y-4 text-[11px] text-primary/60 font-medium">
            ...
            <div className="space-y-8">
              <h5 className="text-[11px] font-bold uppercase tracking-[0.4em] text-accent">Institucional</h5>
              <ul className="space-y-4 text-[11px] text-primary/60 font-medium">
            ...
            <div className="space-y-8">
              <h5 className="text-[11px] font-bold uppercase tracking-[0.4em] text-accent">Redes Sociais</h5>
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-[100] bg-primary/20 backdrop-blur-sm">
          <div className="ml-auto h-full w-full max-w-md bg-background shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
            <div className="flex items-center justify-between p-10 border-b border-accent/10">
              <h3 className="text-3xl font-serif font-bold text-primary">Sacola</h3>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8 no-scrollbar">
              {!cartItems.length ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-6">
                  ...
                  <p className="text-sm text-primary/40 italic font-light">Sua sacola aguarda por peças incríveis.</p>
                </div>
              ) : (
                cartItems.map((item) => (
                  <div key={item.id} className="flex gap-6 p-6 rounded-[2.5rem] bg-white shadow-sm border border-accent/5 hover:shadow-lg transition-all">
                    <div className="h-28 w-20 rounded-2xl overflow-hidden shrink-0 shadow-sm">
                      <img src={item.image} className="h-full w-full object-cover" alt={item.name} />
                    </div>
                    <div className="flex-1 flex flex-col justify-between py-1">
                      <div className="flex justify-between items-start">
                        <h4 className="font-serif font-bold text-primary text-base">{item.name}</h4>
                        <button onClick={() => removeItem(item.id)} className="text-accent/40 hover:text-red-500 transition-colors"><Trash2 className="h-4 w-4" /></button>
                      </div>
                      <div className="flex items-center justify-between">
                        <p className="font-bold text-primary text-base">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(item.price)}</p>
                        <div className="flex items-center gap-3 bg-secondary/40 rounded-full px-4 py-1.5 shadow-inner">
                          <button onClick={() => updateQuantity(item.id, -1)} className="hover:scale-125 transition-transform"><Minus className="h-3 w-3 text-primary" /></button>
                          <span className="text-[11px] font-bold w-4 text-center">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, 1)} className="hover:scale-125 transition-transform"><Plus className="h-3 w-3 text-primary" /></button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-10 bg-white border-t border-accent/10 space-y-8">
              <div className="flex items-center justify-between">
