...
export function ProductManagement() {
  ...
  return (
    <div className="space-y-8">
                ...
                <tr key={p.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="h-14 w-11 rounded-lg overflow-hidden bg-gray-100 shadow-sm border border-gray-50"><img src={p.image} className="h-full w-full object-cover" /></div>
                      <div className="flex flex-col"><span className="font-bold text-primary line-clamp-1 max-w-[200px]">{p.name}</span><span className="text-[10px] text-muted-foreground font-mono uppercase tracking-tight">SKU: {p.sku || p.id?.slice(-6).toUpperCase()}</span></div>
                    </div>
                  </td>
                  <td className="px-8 py-6"><Badge className="bg-secondary text-primary border-none text-[9px] font-bold uppercase px-3">{p.category}</Badge></td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                      <span className="font-bold text-primary">R$ {p.price?.toFixed(2)}</span>
                      {p.cost && <span className="text-[10px] text-emerald-600 font-bold uppercase italic">Lucro: R$ {(p.price - p.cost).toFixed(2)}</span>}
                    </div>
                  </td>
                  <td className="px-8 py-6"><span className="text-sm font-medium">{p.stock} un</span></td>
                  <td className="px-8 py-6">
                    {p.published !== false ? (
                      <div className="flex items-center gap-2 text-emerald-600 font-bold text-[10px] uppercase"><CheckCircle2 className="h-4 w-4" /> Ativo</div>
                    ) : (
                      <div className="flex items-center gap-2 text-amber-500 font-bold text-[10px] uppercase"><XCircle className="h-4 w-4" /> Inativo</div>
                    )}
                  </td>
      ...
      {editingProduct && (
        <EditProductDialog product={editingProduct} open={!!editingProduct} onOpenChange={o => !o && setEditingProduct(null)} />
      )}
    </div>
  );
}
